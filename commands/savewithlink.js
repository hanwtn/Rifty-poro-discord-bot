...
exports.execute = (client, message, args) => {
  ...
  const clientKey = args[1];
  let region = args[0];
  if (region) {
    region = region.toLowerCase();
  }
  if (message.guild) {
    message.reply("Please check your DM to Login.").then(() => {
      message.author
        .send({ embeds: [embed] })
        .catch(() =>
          message.channel.send(
            "Your DM is closed. <@" + message.author.id + ">"
          )
        );
    });
  } else {
    ...
    if (!region || !client.config.region[region]) {
      const regionEmbed = new MessageEmbed()
        ...
        .setFooter(`r!savewithlink to read instructions.`);
...
exports.help = {
  name: "Save With Link",
  aliases: ["savelogin", "login"],
  usage: `savewithlink`,
  info: "Save your in-game profile with Riot Auth.",
};
