...
exports.execute = (client, interaction) => {
  ...
  const url = "https://www.wildriftfire.com/guide/" + client.config.wildriftfire[champion];        // Set website you want to screenshot
  ...
  const Screenshot = async () => {             // Define Screenshot function
    ...
    const loadingEmbed = new Discord.MessageEmbed()
      .setColor("GREEN")
      .setTitle(champion.toUpperCase())
      .setDescription(`<a:Loading:900575343839150170> Loading skill order of *${champion.toUpperCase()}*`)
    ...
    try {
      await page.waitForSelector('#data-wrap > div > div:nth-child(3)', { timeout: 1000 });          // Method to ensure that the element is loaded
    }
    catch (e) {
    ...
    const screenshot = await page.screenshot({ 'clip': { 'x': x, 'y': y, 'width': w, 'height': h } });     // take screenshot of the required area in puppeteer
...
exports.data = new SlashCommandBuilder()
  .setName('so')
  .setDescription('See the skill order of a champion.')
